...
Hardware connections: The PCF8523 should be connected to an I2C bus. If you want
the clock to keep ticking after power is removed, make sure to attach a battery
to the batt pin. The SQW pin can optionally be connected. At this time, this
class does not use that pin for any purpose.
...
/**
Sets the time on the RTC. The various limits (ie, month <= 12) are not checked.

If the RTC chip has a battery backup, then the time will persist across power
cycles.
...
@param weekday The weekday to set, in the range of 0 to 6. You can set and
               interpret this anyway that you want: it's simply incremented when
               the day changes.
...
/**
Get the current time from the RTC chip. Note that the parameters are passed by
reference.
