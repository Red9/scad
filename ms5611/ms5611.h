...
Provides a simple interface to the MS5611 barometer and temperature sensor. Uses
the I2C interface to the sensor.
...
This reset and initializes the sensor, reads the PROM, and begins a conversion.
@a Touch() should be called no sooner than 8.5ms after MS5611 initialization.
...
Test results indicate that the Touch function takes the following amount of time,
at 80MHz in:
+ CMM -Os mode: 49056 cycles (~0.62ms)
+ LMM -Os mode: 22624 cycles (~0.28ms)

@returns true when a new set of data is ready. If result is true, then @a Get()
         should be called.
*/
bool Touch(void);
...
Test results indicate that the Get function takes the following amounts of time,
at 80MHz in when calibrationCalculation == true:
+ CMM -Os mode: 48448 cycles (~0.62ms)
+ LMM -Os mode: 13968 cycles (~0.17ms)

@warning Second order temperature compensation has not been tested! Particularly
the very low temperature compensation!
...
@param calibrationCalculation Perform calculations to calibrate (they involve
                              64 bit integers, so they take a while).
*/
...
private:
	...
	//Calculated temperature and pressure
//	int temperature;
//	int pressure;
//	
	
	//Variables straight from MS5611
//	int D1, D2;
	int64_t C1, C2, C3, C4;
	int C5, C6;
